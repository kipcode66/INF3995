...
CREATE TABLE cachedSong (title VARCHAR(256), artist VARCHAR(256), user_id INTEGER NOT NULL, duration INTEGER, path VARCHAR(256), song_order INTEGER, deleted_by_admin INTEGER, timestamp REAL);
CREATE TRIGGER IF NOT EXISTS add_order_auto_increment AFTER INSERT ON cachedSong
BEGIN UPDATE cachedSong SET song_order = NEW.rowid WHERE rowid = NEW.rowid; END;
COMMIT;
