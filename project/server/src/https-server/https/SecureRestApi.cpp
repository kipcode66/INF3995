...
void SecureRestApi::createSecureDescription_() {
    ...
    superviseurPath
            .route(m_desc.get("/listenoir"))
            .bind(&SecureRestApi::getBlackList_, this)
            .hide();
}
...
void SecureRestApi::superviseurLogin_(const Rest::Request& request, Http::ResponseWriter response) {
    ...
    try {
        ...
        std::async([&]() {
            ...
            if ((db->getUserById(admin.id)).userId == 0 ) {
                logMsg << "Could not Login Admin. Admin with token \"" << admin.id << "\" is not a user.";
                m_logger.err(logMsg.str());
...
void SecureRestApi::getBlackList_(const Rest::Request& request, Http::ResponseWriter response) {
...
std::string SecureRestApi::generateUser_(const User_t& user) {
