...
import ca.polymtl.inf3995.client.controller.event.SendSongEvent
...
class LocalSongAdapter(
    ...
    override fun getView(position: Int, v: View?, viewGroup: ViewGroup?): View {
        ...
        val isSongInQueue = ownedSongs
            .any { s -> s.title == song.title && s.authorName == song.authorName }
        view.send
            .isEnabled = ownedSongs.size < 5 && !isSongInQueue && !isAdmin && (dataProvider[song] <= DataProvider.LocalSongSendState.NOT_SENT)
        ...
        view.send.setOnClickListener {
            val nSendingSong = localSongs.map { song -> dataProvider[song] }
                .filter { state -> state > DataProvider.LocalSongSendState.NOT_SENT && state < DataProvider.LocalSongSendState.SENT }
                .size
            if (nSendingSong < MAX_CONCURRENT_SENDING) {
                ...
                Handler(Looper.getMainLooper()).post {
                    Toast
                        .makeText(appCtx, appCtx.getString(R.string.error_message_too_many_concurrent_song_sent, MAX_CONCURRENT_SENDING), Toast.LENGTH_LONG)
                        .show()
                }
            }
        }
