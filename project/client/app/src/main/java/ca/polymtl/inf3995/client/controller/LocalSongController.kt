...
class LocalSongController(
    ...
    companion object {
        private const val CODE_PERMISSIONS = 133
    }
    ...
    private fun verifyPermissions(activity: Activity) {
        if (activity.checkSelfPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE)
            ...
            activity
                .requestPermissions(arrayOf(android.Manifest.permission.READ_EXTERNAL_STORAGE), CODE_PERMISSIONS)
            return
        }
    ...
    private fun findSong(songsURI: Uri): LocalSongs {
        ...
        val musicFilter = MediaStore.Audio.Media.IS_MUSIC + "!=0 and " + MediaStore.Audio.Media.MIME_TYPE + "==\"audio/mpeg\""
        val cursor = appCtx.contentResolver.query(songsURI, null, musicFilter, null, null)
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    val authorName = cursor
                        .getString(cursor.getColumnIndex(MediaStore.Audio.Media.ARTIST))
                    val songTitle = cursor
                        .getString(cursor.getColumnIndex(MediaStore.Audio.Media.TITLE))
                    val songFileUri = cursor
                        .getString(cursor.getColumnIndex(MediaStore.Audio.Media.DATA))
                    val duration = cursor
                        .getInt(cursor.getColumnIndex(MediaStore.Audio.Media.DURATION))
                    songsList
                        .add(LocalSong(songTitle, authorName, duration, Uri.parse(songFileUri)))
