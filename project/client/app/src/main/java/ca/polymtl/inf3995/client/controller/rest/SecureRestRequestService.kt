...
import ca.polymtl.inf3995.client.controller.VolumeController
import ca.polymtl.inf3995.client.controller.event.EventManager
import ca.polymtl.inf3995.client.controller.event.RequestBlackListReloadEvent
import ca.polymtl.inf3995.client.controller.event.RequestQueueReloadEvent
import ca.polymtl.inf3995.client.controller.event.VolumeRequestEvent
...
import ca.polymtl.inf3995.client.model.*
...
class SecureRestRequestService(
    ...
    private inline fun <reified T> generateRequest(
        method: Int, res: String, body: Any, continuation: Continuation<ResponseData<T>>,
        messages: MutableMap<Int, String>, defaultResponse: T, displayToast: Boolean = false
    ): RESTRequest<T> {
        ...
        return RESTRequest(method, httpsClient.getBaseURL() + "$RESOURCE_URI/$res", Gson().toJson(body), T::class.java,
            ...
            Response.ErrorListener { error ->
                ...
                val resp = ResponseData(error.networkResponse?.statusCode
                                        ?: 0, defaultResponse, error.networkResponse)
                continuation.resume(resp)
            })
    }
        ...
        for (chanson in resp.value.chansons) {
            ...
            list
                .add(Song(chanson.titre, chanson.artiste, duration, chanson.no, if (chanson.proprietaire) null else (chanson.proposeePar
                                                                                                                     ?: ""), chanson.ip, chanson.mac, chanson.id))
        }
        return list
    }
    ...
    suspend fun blockUser(user: User) {
        suspendCoroutine<ResponseData<String>> { continuation ->
            ...
            request
                .retryPolicy = DefaultRetryPolicy(TimeUnit.SECONDS.toMillis(10L).toInt(), 0, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT)
            httpsClient.addToRequestQueue(request)
        }
