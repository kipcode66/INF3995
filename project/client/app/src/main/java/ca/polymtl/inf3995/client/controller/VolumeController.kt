...
class VolumeController(
    ...
    private val volumeProvider =
        object:
            VolumeProviderCompat(VolumeProviderCompat.VOLUME_CONTROL_ABSOLUTE, PERCENTAGE_MAX / PERCENTAGE_MULTIPLIER, PERCENTAGE_INITIAL / PERCENTAGE_MULTIPLIER) {
            ...
            override fun onSetVolumeTo(volume: Int) {
                ...
                if (volume == 0 && currentVolume != 0) {
                    currentVolume = 0
                    eventMgr
                        .dispatchEvent(VolumeChangeRequestEvent(VolumeChangeRequestEvent.Companion.Change.MUTE))
                } else {
                    if (volume != 0 && currentVolume == 0) {
                        eventMgr
                            .dispatchEvent(VolumeChangeRequestEvent(VolumeChangeRequestEvent.Companion.Change.UNMUTE))
                    }
                    currentVolume = Math
                        .max(0, Math.min(volume, PERCENTAGE_MAX / PERCENTAGE_MULTIPLIER))
                    eventMgr
                        .dispatchEvent(VolumeChangeRequestEvent(VolumeChangeRequestEvent.Companion.Change.SET, volume * PERCENTAGE_MULTIPLIER))
                }
            }
            ...
            override fun onAdjustVolume(direction: Int) {
                ...
                when (direction) {
                    AudioManager.ADJUST_RAISE  -> {
                        ++currentVolume
                        currentVolume = Math
                            .max(0, Math.min(currentVolume, PERCENTAGE_MAX / PERCENTAGE_MULTIPLIER))
                        eventMgr
                            .dispatchEvent(VolumeChangeRequestEvent(VolumeChangeRequestEvent.Companion.Change.SET, currentVolume * PERCENTAGE_MULTIPLIER))
                    }
