...
/*
 * Direction pour la configuration des pins
 */
#define IN 0
#define OUT 1
...
int configGPIOs () {
  ...
  Status = XGpioPs_CfgInitialize(&GpioMIO, GPIO_MIOConfigPtr, XPAR_PS7_GPIO_0_BASEADDR);

  if (Status != XST_SUCCESS)
      return XST_FAILURE;

  XGpioPs_SetDirectionPin(&GpioMIO, PB1, IN );
  XGpioPs_SetDirectionPin(&GpioMIO, PB2, IN );
  XGpioPs_SetDirectionPin(&GpioMIO, LD9, OUT );

  XGpioPs_SetOutputEnablePin(&GpioMIO, PB1, FALSE );
  XGpioPs_SetOutputEnablePin(&GpioMIO, PB2, FALSE );
  XGpioPs_SetOutputEnablePin(&GpioMIO, LD9, TRUE );
  ...
  XGpio_SetDataDirection ( &GpioAXI, 2, (u32)-1 );
  XGpio_SetDataDirection ( &GpioAXI, 1, (u32) 0 );
...
void setTimerAndIntr() {
  ...
  Xil_ExceptionInit();
  Xil_ExceptionRegisterHandler( XIL_EXCEPTION_ID_INT, TimerIntrHandler, &Timer);
  Xil_ExceptionEnable();

  /* connecter le timer au gestionnaire */
  XScuGic_Connect(&InterruptController, XPAR_SCUTIMER_INTR, TimerIntrHandler, &Timer);
